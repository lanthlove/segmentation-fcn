...
image_filename = 'data/2007_000068.jpg'
annotation_filename = 'data/2007_000068.png'
...
with sess:
    ...
    downsample_logits_value, train_annotation = sess.run([downsampled_logits_shape, annotation_tensor],
                                              feed_dict=feed_dict_to_use)
...
tf.reset_default_graph()
png = tf.image.decode_png(tf.read_file(annotation_filename), channels=3)
sess = tf.Session()
png_np = sess.run(png)
print(png_np.shape)
for i in range(png_np.shape[0]):
    for j in range(png_np.shape[1]):
        print(png_np.squeeze()[i,j], end=',')
    print('')
